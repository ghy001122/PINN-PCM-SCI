...
2. `BLOCKED_INFRASTRUCTURE`: 已知 endpoint 返回 `Connection refused`。等待用户重启 AutoDL 并提供当前 host/port 与页面实时单价；campaign 科学授权继续有效且运行计数仍为 0。
3. `PENDING_AFTER_RESTART`: 从 scratch 执行 E1 clean-coupling exploration；实时只读观察，完成后回收、哈希核验并立即关闭 AutoDL。
4. `PENDING`: 关机验证后，本地运行 frozen nominal evaluator；按预声明结果唯一选择 confirmation、对应 E2 或终止。
5. `CONDITIONAL`: 若 E2 有 material phase signal 但未 competent，执行同一 E2 身份加 500 full-joint updates 的 E3；否则终止 pure-scratch。
6. `CONDITIONAL`: 首条完整 competence signal 后，仅执行一次完全冻结、from-scratch confirmation。
7. `PENDING`: 写 compact evidence、ledger、closeout 与 terminal authority state，精确白名单提交并推送 main。
