...
def train(cfg,device):
    ...
    for arm in ARMS:
        ...
        exp,cal=inputs(cfg,arm,device)
        actual_device=str(next(exp.model.correction.parameters()).device)
        if torch.device(actual_device)!=torch.device(device):
            raise RuntimeError('Requested training device was not propagated')
        ...
        try:
            ...
            terminal=dict(status='VALID_FIXED_ENDPOINT',arm=arm,adam_updates=600,lbfgs=result,
                ...
                requested_device=device,actual_device=actual_device)
