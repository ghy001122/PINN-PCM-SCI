"""Build the hash-bound reference-blind LF3 AutoDL source bundle."""
...
MANIFEST = ROOT / "cloud" / "phk_v23_lf3_autodl" / "deployed-source-manifest.json"
MEDIUM = ROOT / "outputs" / "runs" / "20260828T-phk-v21-s1-q-04-nominal-medium" / "result-intent-04.npz"
INITIAL_CHECKPOINT = ROOT / "outputs" / "runs" / "20260903T152501Z-phk-v23-lf1-b-event-replay-dc091be-er1" / "checkpoint-b0-step-1200.pt"
STATIC_FILES = (
    "cloud/phk_v23_lf3_autodl/preflight.py",
    "cloud/phk_v23_lf3_autodl/README.md",
...
def build(*, qualification_path: Path, archive_path: Path, base_commit: str) -> dict[str, object]:
    ...
    if len(files) != len(set(files)):
        raise ValueError("LF3 bundle contains duplicate paths")
    bindings: dict[str, str] = {}
    for relative in files:
        path = ROOT / relative
        if not path.is_file():
            raise FileNotFoundError(f"LF3 source file absent: {relative}")
    ...
    source_identity = f"LF3-BUNDLE-{aggregate}"
    if not MEDIUM.is_file() or not INITIAL_CHECKPOINT.is_file():
        raise FileNotFoundError("LF3 separately uploaded input absent")
    qualification_payload = json.loads(qualification.read_text(encoding="utf-8"))
    if qualification_payload.get("status") != "LF3_CPU_QUALIFICATION_PASS" or qualification_payload.get("gpu_execution_authorized_by_cpu_gate") is not True:
        raise PermissionError("LF3 bundle requires passed CPU qualification")
    manifest = {
        "schema_id": "phk-v23-lf3-deployed-source-manifest-v1",
        "source_identity": source_identity,
        "identity_definition": "SHA256_OF_SORTED_PATH_EQUALS_UPPERCASE_SHA256_LINES",
        "base_commit": base_commit,
        "workspace_scope": "LF3_EXACT_ALLOWLIST_UNRELATED_DIRTY_EXCLUDED",
        "training_inputs": {
            "medium": {"path": MEDIUM.relative_to(ROOT).as_posix(), "sha256": _sha256(MEDIUM), "size_bytes": MEDIUM.stat().st_size},
            "initial_checkpoint": {"path": INITIAL_CHECKPOINT.relative_to(ROOT).as_posix(), "sha256": _sha256(INITIAL_CHECKPOINT), "size_bytes": INITIAL_CHECKPOINT.stat().st_size},
        },
        "cpu_qualification": {"path": qualification_relative, "sha256": _sha256(qualification), "size_bytes": qualification.stat().st_size},
        "files": dict(sorted(bindings.items())),
    }
    if MANIFEST.exists():
        raise FileExistsError(f"refusing to replace LF3 source manifest: {MANIFEST}")
    MANIFEST.write_text(json.dumps(manifest, indent=2, sort_keys=True, allow_nan=False) + "\n", encoding="utf-8", newline="\n")
    archive = Path(archive_path).resolve()
    archive.parent.mkdir(parents=True, exist_ok=True)
    if archive.exists():
        raise FileExistsError(f"refusing to replace LF3 archive: {archive}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(json.dumps(build(qualification_path=args.qualification, archive_path=args.archive, base_commit=args.base_commit), sort_keys=True))
    return 0
