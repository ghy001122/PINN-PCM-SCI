"""Build the exact LF2 source manifest and portable source archive locally."""
...
MANIFEST = ROOT / "cloud" / "phk_v23_lf2_autodl" / "deployed-source-manifest.json"
MEDIUM = (
    ROOT
    / "outputs"
    / "runs"
...
INITIAL_CHECKPOINT = (
    ...
    / "checkpoint-b0-step-1200.pt"
)
STATIC_FILES = (
    "cloud/phk_v23_lf2_autodl/preflight.py",
    "cloud/phk_v23_lf2_autodl/README.md",
...
def build(
    *, qualification_path: Path, archive_path: Path, base_commit: str
    ...
    try:
        ...
        raise PermissionError("LF2 qualification must be inside repository") from exc
    files = (*STATIC_FILES, qualification_relative)
    if len(set(files)) != len(files):
        raise ValueError("LF2 source manifest contains duplicate paths")
    bindings: dict[str, str] = {}
    for relative in files:
        exact = ROOT / Path(relative)
        if not exact.is_file():
            raise FileNotFoundError(f"LF2 source file is absent: {relative}")
    ...
    source_identity = f"LF2-BUNDLE-{aggregate}"
    if not MEDIUM.is_file() or not INITIAL_CHECKPOINT.is_file():
        raise FileNotFoundError("LF2 separately uploaded training input is absent")
    qualification_payload = json.loads(qualification.read_text(encoding="utf-8"))
    if (
        qualification_payload.get("status") != "LF2_CPU_QUALIFICATION_PASS"
        or qualification_payload.get("gpu_execution_authorized_by_cpu_gate") is not True
    ):
        raise PermissionError("LF2 bundle requires a passed CPU qualification")
    manifest = {
        "schema_id": "phk-v23-lf2-deployed-source-manifest-v1",
        "source_identity": source_identity,
        "identity_definition": "SHA256_OF_SORTED_PATH_EQUALS_UPPERCASE_SHA256_LINES",
        "base_commit": str(base_commit),
        "workspace_scope": "LF2_EXACT_FILE_ALLOWLIST_UNRELATED_DIRTY_WORKTREE_CONTENT_EXCLUDED",
        "training_inputs": {
            "medium": {
            ...
            "initial_checkpoint": {
                "path": INITIAL_CHECKPOINT.relative_to(ROOT).as_posix(),
    ...
    if MANIFEST.exists():
        raise FileExistsError(f"refusing to replace LF2 source manifest: {MANIFEST}")
    MANIFEST.write_text(
    ...
    if archive.exists():
        raise FileExistsError(f"refusing to replace LF2 archive: {archive}")
    ...
    return {
        ...
        "initial_checkpoint_separate_upload": True,
    }
