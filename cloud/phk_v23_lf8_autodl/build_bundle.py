"""Build the activation-commit and hash-bound-input LF8 cloud bundle."""
...
MANIFEST = ROOT / "cloud/phk_v23_lf8_autodl/deployed-source-manifest.json"
LOCK = ROOT / "configs/phk_v23/lf8_competence_filter_write_allowlist_lock.json"
...
LF8_CONTRACTS = {
    role: f"configs/phk_v23/{role}_contract_lf8_competence_filter.json"
...
STATIC_FILES = (
    ...
    *LF8_CONTRACTS.values(),
    ...
    "pinn_pcm_sci/phk_v23_lf8_qualification.py",
...
def _verify_activation_commit(base_commit: str, files: Sequence[str]) -> None:
    if len(base_commit) != 40 or any(c not in "0123456789abcdefABCDEF" for c in base_commit):
        raise ValueError("LF8 base commit must be a full Git object id")
    if _git("rev-parse", "HEAD").decode().strip().lower() != base_commit.lower():
        raise PermissionError("LF8 bundle must be built at the activation commit")
    for relative in files:
        path = ROOT / relative
        if not path.is_file():
            raise FileNotFoundError(f"LF8 committed source absent: {relative}")
        if _git("show", f"{base_commit}:{relative}") != path.read_bytes():
            raise PermissionError(f"LF8 source is not exact activation-commit content: {relative}")


def _locked_qualification_paths() -> tuple[Path, Path]:
    lock = _read_object(LOCK)
    paths = lock.get("timestamped_paths")
    if not isinstance(paths, Mapping):
        raise ValueError("LF8 lock lacks timestamped qualification paths")
    artifact_value = paths.get("cpu_artifact")
    manifest_value = paths.get("cpu_manifest")
    if not artifact_value or not manifest_value:
        raise ValueError("LF8 lock lacks CPU artifact/manifest paths")
    result = ((ROOT / str(artifact_value)).resolve(), (ROOT / str(manifest_value)).resolve())
    for path in result:
...
def _verify_frozen_inputs() -> None:
    for path, digest in {
        ...
        if not path.is_file() or _sha256(path) != digest:
            raise PermissionError(f"LF8 frozen input absent or hash-drifted: {path}")
    ...
    if (
        ...
        raise PermissionError("LF8 materialized-ledger identity drift")
    data = _read_object(ROOT / LF8_CONTRACTS["data"])
    source = data.get("training_source", data.get("medium_audit_source", {}))
    dev_r = data.get("initial_DEV_R", data.get("initial_dev_r", {}))
    ...
    if (
        ...
        raise PermissionError("LF8 data-contract cloud-input binding drift")


def _verify_qualification(path: Path, manifest_path: Path) -> dict[str, Any]:
    qualification = _read_object(path)
    if (
        qualification.get("schema_id") != "phk-v23-lf8-cpu-qualification-v1"
        or qualification.get("task_id") != TASK_ID
        or qualification.get("gate_outcome") != "LF8_CPU_QUALIFICATION_PASS"
        ...
        raise PermissionError("LF8 bundle requires the exact passed zero-update CPU qualification")
    contracts = {
        role: {"path": relative, "sha256": _sha256(ROOT / relative)}
        for role, relative in LF8_CONTRACTS.items()
    }
    ...
    if any(str(ledger.get(key, "")).upper() != value for key, value in {
        ...
        raise PermissionError("LF8 CPU qualification ledger identity drift")
    ...
    if (
        ...
        raise PermissionError("LF8 CPU qualification manifest drift")
...
def build(*, qualification_path: Path, qualification_manifest_path: Path,
    ...
    if (qualification, qualification_manifest) != _locked_qualification_paths():
        raise PermissionError("LF8 qualification inputs differ from locked paths")
    if not qualification.is_file() or not qualification_manifest.is_file():
        raise FileNotFoundError("LF8 qualification artifact or manifest absent")
    ...
    if len(committed_files) != len(set(committed_files)):
        raise ValueError("LF8 bundle contains duplicate committed paths")
    ...
    identity = "LF8-BUNDLE-" + hashlib.sha256("".join(lines).encode("ascii")).hexdigest().upper()
    manifest = {
        "schema_id": "phk-v23-lf8-deployed-source-manifest-v1",
        "task_id": TASK_ID,
        "source_identity": identity,
        "identity_definition": "SHA256_OF_BASE_COMMIT_AND_SORTED_SOURCE_AND_INPUT_SHA_LINES",
        "base_commit": base_commit,
        "workspace_scope": "LF8_ACTIVATION_COMMIT_PLUS_EXACT_HASH_BOUND_INPUTS_UNRELATED_DIRTY_EXCLUDED",
        "scientific_order": [
    ...
    if MANIFEST.exists():
        raise FileExistsError(f"refusing to replace LF8 deployment manifest: {MANIFEST}")
    ...
    members = [*(ROOT / relative for relative in committed_files), MEDIUM, DEV_R_CHECKPOINT, LEDGER, LEDGER_MANIFEST, MANIFEST]
