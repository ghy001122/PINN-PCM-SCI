...
Figure generation reads existing `.npz`, JSON, and contract carriers and performs no solver or training work. Use any local Python environment satisfying `paper/paper_v2/figures/requirements-figures.txt` (the recorded render used Python 3.8.4, NumPy 1.26.4, and Matplotlib 3.8.4):

~~~powershell
python paper\paper_v2\figures\generate_figures.py
~~~

The generator rewrites only `paper/paper_v2/figures/figure-01...figure-06` in PNG/PDF form and the derived CSV files in `paper/paper_v2/figures/data/`. It verifies the terminal summary hash before drawing.
