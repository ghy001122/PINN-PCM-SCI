# Before Training the PINN: Preregistered Source and Pre-Oracle Qualification for a Synthetic Electrothermal Defect-Transport Benchmark

**Manuscript type:** Clean-room benchmark and method-limits study  
**Evidence identity:** Fully transparent synthetic benchmark; no source-aligned replay; no experimental validation  
...
Physics-informed neural-network studies of switching devices require more than a plausible multiphysics model: they require a legally and technically usable object specification, a numerically qualified reference solver, a stable event definition, and a claim boundary that survives failure. We report a project-local, pre-result-registered qualification study designed to establish those prerequisites before any neural training. Two source-aligned routes were assessed against fixed contracts. A COMSOL 6.4 memristor example did not establish the required research-use and independent-reference-output conditions and did not close several model-tree fields. A published PCMO reaction-drift model was a lumped point-device construction whose current response depended on an unavailable Sentaurus lookup table. The preregistered fallback was therefore a fully transparent two-dimensional axisymmetric synthetic electrothermal defect-transport benchmark. Its governing equations, geometry, parameters, waveform, event criteria, discretization, tolerances, 13-intent qualification ladder, and no-rescue rule were frozen and hash-bound before the first numerical result.

The zero-drive Q0 implementation guard completed 400 time steps in 8.38404590007849 s. It preserved the defect fraction at exactly 0.5, kept temperature within 299.9999999999985–300.00000000000034 K, and produced zero recorded mass drift, no-flux residual, heat-balance residual, and terminal-current mismatch. This was an implementation guard, not an oracle result. The first driven QN qualification intent then stopped before producing a case field because the transport Newton solve exceeded its frozen 20-iteration limit. A separately labeled non-scientific minimal fixture reproduced the failure class: an initial scaled residual of $1.5106745331996967\times10^{-3}$ decreased with 20 accepted half steps to $1.4406930175716191\times10^{-9}$, still above the frozen $10^{-10}$ threshold, while a directional finite-difference check agreed with the analytic Jacobian to $1.7339861280712171\times10^{-10}$ in relative infinity norm.

The terminal result is a numerical-contract No-Go for this frozen solver configuration. It is not evidence that the physical equations are unsolvable, that a driven event exists or does not exist, or that any PINN succeeds or fails. No oracle, event, neural training, GPU development, or formal comparison was reached. The study demonstrates a failure-preserving route from source audit to claim-bounded termination and provides a reproducible negative benchmark qualification record that prevents numerical infrastructure from being misreported as machine-learning evidence.
...
1. It gives an executable pre-oracle workflow in which source and legal-access questions are separated from physical-model completeness, numerical validity, event qualification, and machine-learning claims.
2. It reports the complete terminal evidence from one frozen synthetic electrothermal defect-transport qualification attempt, including the successful zero-drive guard, the first driven execution failure, exact resource accounting, and a separately classified diagnostic localization.
3. It provides a claim-boundary template for preserving a negative result without expanding it into a claim about the physical model, the existence of switching events, or PINN performance.

This article deliberately does **not** report a trained PINN. The reference-solver gate failed before an oracle was established, so raw neural baselines, conditional architectures, out-of-distribution tests, and sealed formal comparisons were not scientifically admissible under the preregistered contract. The resulting paper is therefore a clean-room benchmark and method-limits study, not a positive method paper and not an experimental device study.
...
8. **The code identity was a dirty local working tree.** Run manifests recorded the working-tree revision and dirty state. Reproducibility therefore relies on the delivered file hashes and package inventory rather than on a clean public commit.
...
[C12] Zhuoyuan Wang, Raffaele Romagnoli, Saviz Mowlavi, and Yorie Nakahira. **Physics-Informed Deep B-Spline Networks (PI-BSNet).** AI and PDE Workshop at ICLR 2026, poster; exact reviewed carrier OpenReview x1TWOnfTX8. <https://openreview.net/forum?id=x1TWOnfTX8>; <https://openreview.net/pdf?id=x1TWOnfTX8>.
