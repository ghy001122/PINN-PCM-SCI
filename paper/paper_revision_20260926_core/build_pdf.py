...
styles = {
    ...
    "suppref": ParagraphStyle("suppref", fontName="Body", fontSize=8.6, leading=10.3, spaceAfter=3.5, splitLongWords=True),
...
def inline(text):
    ...
    symbols = ("A_w", "S_w", "u_θ", "T_θ", "φ_θ", "σ_θ", "V_θ", "h_T", "h_φ", "h_V", "A_e", "ω_i", "q_i", "P_J", "L_obs", "L_BC", "L_IC", "J_Tφ", "E_φ", "E_T", "E_V", "E_I", "E_P", "E_W", "B_E", "D_E", "P_E", "F_raw", "F_full", "F_bal", "D_C", "P_kappa", "ε_logit", "d_scale", "T_c", "m_c", "m_h", "w_M")
    ...
    for symbol in ("E_C", "E_R", "R_ξ", "c_g", "ell_θ", "e_c", "e_b", "δ_d"):
        base, sub = symbol.split("_", 1)
        if base == "ell": base = '<font face="Sans">ℓ</font>'
        text = re.sub(r"(?<![\w/])"+re.escape(symbol)+r"(?![\w/])", base+"<sub>"+sub+"</sub>", text)
...
def make_table(rows):
    ...
    if numeric_revision and n == 9 and rows[0][-1] == "P (%)":
        widths = [42, 31, 30, 57, 66, 56, 67, 56, 56]
        widths = [w*WIDTH/sum(widths) for w in widths]
    elif numeric_revision and n == 9:
...
def footer(canvas, doc):
    ...
    canvas.drawString(52, 27, label + "  |  Integrated revision, 25 September 2026")
...
for name, blocks in manifest["documents"].items():
    ...
    while i < len(blocks):
        b = blocks[i]
        if b["type"] == "heading":
            if b["text"].startswith("S18. Fixed-array mechanism diagnostics"):
                # Keep this graphic-led section title with its first full panel.
