...
LF6 的 DEV-U/DEV-R 各完成 400 个 matched phase-only updates；DEV-R 通过 safety 并按冻结规则进入 1200-step label-free P0，但两臂均未通过 strict，故没有 rank-specific mechanism increment。P0 将 fixed-blind physics objective 降至入口的 `0.0128142`，却使 V/T/phase/topology preservation 分别恶化 `28.62/52.60/25.84/20.03` 倍并抹除事件，因此这是有效的 physics-forgetting 负结果，不是 PINN Pareto 或 candidate。产物已回收核验，实例已关机；stress 保持 sealed/unread。
...
- 当前关闭决定：[ADR 0066](docs/adr/0066-close-phk-v23-lf6-event-frontier-pilot.md)
- LF6 终局：[terminal closeout](docs/experiment/2026-09-06-phk-v23-lf6-terminal-closeout.md)
- LF6 激活决定：[ADR 0065](docs/adr/0065-activate-phk-v23-lf6-event-frontier-pilot.md)
