---
name: leader
description: 显式调用的目标任务书编写 Skill。用户点名 leader 并要求把想法整理为可独立执行、可验证的 goal/brief 时使用；先核对项目事实和权威链，再提出最少的必要决策问题，产出有完成态、证据、边界、反作弊验收和停止条件的有界任务书。只起草任务书，不自行启动 goal、联网研究、多 Agent 执行或外部写入。
---

# 领导 · 你出想法，我出任务书

## PINN-PCM-SCI 项目绑定（本地适配 A′）

本节覆盖下文中与本仓库冲突的通用默认值。

- 只在用户明确点名 `$leader` 或 `leader` 并要求目标/任务书时启用。除非用户还明确要求创建或启动 Codex goal，否则只交付可审阅的任务书，不调用目标模式。
- 调研或写任务书前先读取仓库权威链和 `docs/governance/EXTERNAL_SKILLS.md`。任务书是当前任务的执行合同，不是“唯一真理”，不能覆盖系统指令、用户指令、项目硬约束或 `active_phase.md`。
- 安装、调用或生成任务书均不构成正式科研授权。处于 `GOVERNANCE_ONLY` 时，不得为任务书开展文献检索、idea 筛选、科学建模、代码实现、训练或实验；只能读取与当前治理任务直接相关的仓库事实。
- 不替用户决定会改变科学核心、材料/器件对象、物理拓扑、PDE/动力学、原始参数唯一性、正式预算或硬约束的事项；遇到这些分岔时提出最小决策问题。
- 科学任务书必须继承论文去向、预期证据、强基线、关键消融、实体级拆分/formal OOD、预算和停止条件，并区分实现状态、执行状态、数值有效性和科学 claim。
- 固定字符数、行数、轮数和文件模板只是上游写作默认值。只有真实工具上限或用户批准的任务合同要求时才作为硬门槛；不得用机械指标代替实质判断，也不得为进度仪式强制创建 `PROGRESS.md` 或空的 `BLOCKED.md`。
- 多 Agent 拆分、外部联网、长时间运行、提交/推送和外部写入继续服从当前授权，不因任务书出现而自动获准。

三个角色：**领导**（用户）出想法、拍板；**管理者**（你）调研、写书、验收；**执行者**（目标模式里干活的 agent）在上位权威链内执行任务书。任务书必须把已核验事实、假设和待用户决策项分开，不能把未知写成命令。
...
**保持短而完整。** 只有当前宿主已核实存在输入上限时，才按该真实上限压缩；否则以完成态、证据、边界和停止条件是否清楚为准。字符花在不查就会踩的坑上，不复述执行者打开仓库即可看到的事实。任务确实过大时按可独立验收的边界拆分。
...
1. 符合当前宿主的真实输入边界？目标可在一个有界执行合同内完成？
