...
- Read the repository authority chain before every use. Installing or explicitly naming this Skill does not authorize novelty research; while `active_phase.md` remains `GOVERNANCE_ONLY`, do not run the procedure.
- Do not infer a missing research problem or novelty when the inference would change the scientific claim. Ask the minimum decision question or mark the check blocked.
- Use the project-installed `paper-search` under `.agents/skills/paper_search`; Claude-specific `TaskCreate`, `WebFetch`, `Read`, `.claude`, and `${CLAUDE_PROJECT_DIR}` references below are illustrative rather than executable contracts in Codex.
- Store every step, PDF, extracted text, query, and verdict under one unique, predeclared run directory; never write generic `step1.md` through `step7.md`, `papers/`, or `allinone.md` at repository root.
- Do not execute the bundled `fetch_paper.sh` on native Windows. Use an approved PDF acquisition/inspection path with resolved-target boundary checks, and distinguish full-text reads from abstract-only fallbacks.
- Model recall is an `UNVERIFIED_LEAD`. A zero-result search is not global novelty evidence; the strongest permissible statement is that no collision was found within the declared databases, queries, date window, successful connectors, and full-text coverage.
- Numeric overlap levels are an organizational aid, not evidence of novelty, priority, paper quality, or route admission. Project claim and evidence rules control the final wording.
