---
name: neat-freak
description: 显式调用的项目知识与治理收尾 Skill。仅当用户点名 neat-freak、洁癖或 /neat，并要求同步本次真实改动影响的文档、规则、获准记忆或工程状态时使用；服从项目权威链，默认只读对待 Codex 生成记忆，不做隐式清理或形式化全仓审计。
metadata:
  version: "3.0.0"
...
- 只在用户明确点名 `$neat-freak`、`neat-freak`、`洁癖` 或 `/neat` 时启用。安装或调用本 Skill 不扩大 `active_phase.md` 的授权范围。
- 每次使用先读取仓库权威链和 `docs/governance/EXTERNAL_SKILLS.md`。`AGENTS.md` 与项目研究规范不是可由代码现状自动改写的普通文档；研究硬约束只有用户明确修改时才可改变。
- 当前 `GOVERNANCE_ONLY` 阶段只允许收尾用户已明确授权的治理改动，不得借收尾启动文献检索、idea 筛选、建模、训练、实验或科学主张。
- 只核对本次真实改动会影响的事实面和文件。不得为流程完整而全仓枚举、统计行数/字节/文件数、重复哈希或重复同类审计；上游全量 Bash 盘点脚本与评测夹具因此未纳入本地安装。
- Codex 机器生成记忆保持只读。仅当用户明确要求更新记忆时，才按当前宿主规定写 correction input；不得直接改写 `MEMORY.md`、memory summary 或 rollout summary。
- 删除、清场、外部写入、提交、推送和发布均服从当前系统、用户与项目授权。上游固定的二次确认流程不额外收紧或放宽现场权限；执行破坏性动作前仍须解析并核对精确目标。
- 治理同步、Git 状态和测试结果只证明工程状态，不得表述为学术证据或科学结论。
...
- 使用当前 Windows 工具对本次改动涉及的规则、文档、Git/worktree 和运行入口做针对性只读盘点。
- 仅在真实平台上限已核实且读数会改变动作时记录体量；其他情况下不统计行数、字节数或文件数。
...
必须明确列出 `pending`、`out-of-scope` 和未消除的 warning，并在存在待清场现场时写明复核现场是否保留；不能用「保证干净」掩盖它们。完成获准清场后，只补充汇报实际删除项、针对性清场核验和残留 warning，不重写第一阶段的完整结果。只有真实平台上限会改变动作时才报告容量读数。

## 最终自检

- [ ] 每个事实面都有状态（含 `not-applicable`），没有把未验证写成完成。
- [ ] 本次改动影响的文件已定位、阅读并作出「改/不改」判断；没有为形式做全仓机械枚举。
